...
public class KeyFinder extends Enchant {
	...
	public KeyFinder(String name, int maxLevel, int cost, boolean expodential, String type, String tier, String string) {
		super(name, maxLevel, cost, expodential, type, tier, string);
	}
	
	public static void runKeyFinder(ItemStack inHand, Player p)
	{
		Random r = new Random();
		...
		Enchant keyfinder = MainUtils.getEnchant("keyfinder");
		if (keyfinder == null)
		{
			return;
		}
		...
		if (ItemUtil.containsEnchant(keyfinder, inHand))
		{
			...
			KeyChance run = handler.new KeyChance(level,p,keyfinder);
			run.run();
		} else {
			return;
		}
